...
(async () => {
  for (const project of ['playwright-core', 'playwright']) {
    const lines = [];
    lines.push(`ArpitSureka/${project.replace('playwright', 'playwright-sap')}

Playwright built by Microsoft is a really great project. This project would have never been possible without contibutions made by Microsoft Corporation to the open source community.
Hats off 🫡👏 and Thanks to all the devlopers and people at Microsoft for building this great project.
https://github.com/microsoft/playwright/graphs/contributors


This project incorporates components from Microsoft Playwright project. The original copyright notices and the licenses under which Arpit-Sureka received such components are set forth below. Arpit-Sureka reserves all rights not expressly granted herein, whether by implication, estoppel or otherwise.
NOTICES AND INFORMATION FOR Playwright BEGIN HERE
