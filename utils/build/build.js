...
class ProgramStep extends Step {
  ...
  async run() {
    ...
    console.log(
      `==== Running ${step.command} ${step.args.join(" ")} in ${
...
async function runWatch() {
  ...
  function runOnChange(onChange) {
    ...
    chokidar
      .watch(
        [...paths, ...mustExist, onChange.script].filter(Boolean).map(filePath)
      )
      .on("all", reschedule);
    callback();
  }
  ...
  for (const step of steps) {
    if (!step.concurrent) await step.run();
  }

  for (const step of steps) {
    if (step.concurrent) step.run().catch((e) => console.error(e));
  }
  for (const onChange of onChanges) runOnChange(onChange);
}
...
async function runBuild() {
  ...
  for (const onChange of onChanges) runOnChangeStep(onChange);
}
...
for (const bundle of bundles) {
  ...
  if (bundle.modulePath === "packages/playwright-client") continue;
...
class EsbuildStep extends Step {
  ...
  async run() {
    if (watchMode) {
      ...
      console.log(
        "==== Running esbuild:",
        this._relativeEntryPoints().join(", ")
      );
      const start = Date.now();
...
for (const pkg of workspace.packages()) {
  ...
  if (["@playwright/client"].includes(pkg.name)) continue;
...
function copyXdgOpen() {
  ...
  fs.copyFileSync(
    filePath(
      "packages/playwright-core/bundles/utils/node_modules/open/xdg-open"
    ),
    path.join(outdir, "xdg-open")
  );
...
for (const bundle of bundles) {
  ...
  const options = {
    ...
    entryPoints: bundle.entryPoints.map((e) =>
      path.join(filePath(bundle.modulePath), e)
    ),
...
async function copyDirectory(sourcePath, destinationPath) {
  function walk(currentDir) {
    ...
    for (const entry of entries) {
      ...
      const relPath = path.relative(sourcePath, srcPath).replace(/\\/g, "/"); // Use forward slashes for minimatch

      // Skip the destination directory if it's inside the source
      if (path.resolve(srcPath) === destinationPath) {
        continue;
      }
      ...
      if (stats.isDirectory()) {
        ...
        const destPath = path.join(destinationPath, relPath);
  ...
  try {
    walk(sourcePath);
    console.log(
      `Folder '${sourcePath}' copied to '${destinationPath}' successfully!`
    );
  } catch (err) {
...
function copyUI5Extension() {
  const outdir = filePath("packages/playwright-core/lib/server/sap/ui5Extension/");
  if (!fs.existsSync(outdir)) 
    fs.mkdirSync(outdir, { recursive: true });

  copyDirectory( filePath("packages/playwright-core/src/server/sap/ui5Extension/"), outdir,);
  console.log("==== Copied UI5 Extension", path.join(outdir, "xdg-open"));
}

// Copy xdg-open after bundles 'npm ci' has finished.
steps.push(new CustomCallbackStep(copyUI5Extension));
...
// Generate channels.
onChanges.push({
  inputs: ["packages/protocol/src/protocol.yml"],
    ...
    steps.push(
      new ProgramStep({
        ...
        args: [
          ...
          "--preserveWatchOutput",
          "-p",
          quotePath(filePath(`packages/${webPackage}`)),
        ],
        shell: true,
        concurrent: true,
      })
