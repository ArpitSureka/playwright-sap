...
export function isElementVisible(element: Element): boolean {
  const res =  box(element).visible;

  // Custom for sap ui5
  if (res) {
    const classInVisible = element.classList.value.toLowerCase().includes('invisible') || element.classList.value.toLowerCase().includes('hidden');
    const idInvisible = element.id.toLowerCase().includes('hidden') || element.id.toLowerCase().includes('hidden');
    if (idInvisible || classInVisible)
      return false;
  }
